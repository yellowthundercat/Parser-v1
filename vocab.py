...
class Vocab(Configurable):
    ...
    def _finalize(self):
        ...
        with tf.device('/cpu:0'):
            with tf.compat.v1.variable_scope(self.name):
                self.trainable_embeddings = tf.compat.v1.get_variable('Trainable',
                                                                      shape=(len(self._str2idx), embed_size),
    ...
    def embedding_lookup(self, inputs, pret_inputs=None, moving_params=None):
        ...
        embed_input = tf.nn.embedding_lookup(params=trainable_embeddings, ids=inputs)
        if moving_params is None:
            tf.compat.v1.add_to_collection('Weights', embed_input)
        if self.use_pretrained and pret_inputs is not None:
            if self.testPrint < 10:
                print('output', embed_input, tf.nn.embedding_lookup(params=self.pretrained_embeddings, ids=pret_inputs))
            return embed_input, tf.nn.embedding_lookup(params=self.pretrained_embeddings, ids=pret_inputs)
        else:
            if self.testPrint < 10:
    ...
    def weighted_average(self, inputs, moving_params=None):
        ...
        embed_input.set_shape(
            [tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(self.embed_size)])
        if moving_params is None:
            tf.compat.v1.add_to_collection('Weights', embed_input)
        return embed_input
