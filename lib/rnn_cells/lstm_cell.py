    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.compat.v1.variable_scope(scope or type(self).__name__):
            ...
            with tf.compat.v1.variable_scope('Linear'):
                biases = tf.compat.v1.get_variable('Biases', [4 * self.output_size],
            ...
            if self.hidden_include_prob < 1 and self.moving_params is None:
                hidden_mask = tf.nn.dropout(tf.ones_like(hidden_t),
                                            1 - (self.hidden_include_prob)) * self.hidden_include_prob
                hidden_t = hidden_mask * hidden_t + (1 - hidden_mask) * hidden_tm1
            if self.cell_include_prob < 1 and self.moving_params is None:
                cell_mask = tf.nn.dropout(tf.ones_like(cell_t), 1 - (self.cell_include_prob)) * self.cell_include_prob
