...
class BaseOptimizer(Configurable):
    ...
    def minimize(self, loss, name=None):
        ...
        grads = tf.gradients(ys=loss, xs=var_refs,
                             gate_gradients=True,
        ...
        with tf.compat.v1.op_scope([], name, self._name) as name:
            ...
            for cache in caches:
                ...
                with tf.compat.v1.name_scope("update_" + x_tm1.op.name), tf.device(x_tm1.device):
                    if isinstance(g_t, tf.Tensor):
                        cache['g_t'] = tf.select(tf.math.is_finite(g_t), g_t, tf.zeros_like(g_t))
                        self._apply_dense(cache)
                    else:
                        cache['g_t'] = tf.select(tf.math.is_finite(g_t.values), g_t.values, tf.zeros_like(g_t.values))
