...
def linear(inputs, output_size, add_bias=True, n_splits=1, initializer=None, scope=None, moving_params=None):
    ...
    with tf.compat.v1.variable_scope(scope or 'Linear'):
        ...
        new.set_shape(
            [tf.compat.v1.Dimension(None) for _ in xrange(len(shapes[0]) - 1)] + [tf.compat.v1.Dimension(output_size)])
        if n_splits > 1:
...
# ===============================================================
def diagonal_bilinear(inputs1, inputs2, output_size, add_bias2=True, add_bias1=True, add_bias=False, initializer=None,
                      scope=None, moving_params=None):
    """"""
    ...
    with tf.compat.v1.variable_scope(scope or 'Bilinear'):
        ...
        with tf.compat.v1.variable_scope('Bilinear'):
            bilin = linear(inputs, output_size, add_bias=add_bias, initializer=initializer, scope=scope,
                           moving_params=moving_params)
        with tf.compat.v1.variable_scope('Linear1'):
            lin1 = linear(inputs1, output_size, add_bias=False, initializer=initializer, scope=scope,
                          moving_params=moving_params)
            lin1 = tf.expand_dims(lin1, 2)
        with tf.compat.v1.variable_scope('Linear2'):
            lin2 = linear(inputs2, output_size, add_bias=False, initializer=initializer, scope=scope,
                          moving_params=moving_params)
            lin2 = tf.expand_dims(lin2, 1)

        bilin = tf.transpose(a=bilin + lin1 + lin2, perm=[0, 1, 3, 2])
...
def layer_norm(inputs, beta_start=0, gamma_start=1, scope=None, moving_params=None):
    ...
    with tf.compat.v1.variable_scope(scope or "Layer_norm"):
        ...
        beta = tf.compat.v1.get_variable('Beta', shape=[],
                                         initializer=tf.compat.v1.constant_initializer(beta_start))
        if moving_params is not None:
...
def broadcast_add(inputs1, inputs2):
    ...
    inputs2_shape = tf.shape(input=inputs2)
    inputs1 = tf.transpose(a=inputs1, perm=[0, 2, 1])
    inputs2 = tf.transpose(a=inputs2, perm=[0, 2, 1])
