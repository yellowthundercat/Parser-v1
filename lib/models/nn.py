...
class NN(Configurable):
    ...
    def embed_concat(self, word_inputs, tag_inputs=None, rel_inputs=None):
        ...
        if self.moving_params is None:
            ...
            noise_shape = tf.stack([tf.shape(input=word_inputs)[0], tf.shape(input=word_inputs)[1], 1])

            if word_keep_prob < 1:
                word_mask = tf.nn.dropout(tf.ones(noise_shape), 1 - (word_keep_prob)) * word_keep_prob
            else:
                word_mask = 1
            if tag_inputs is not None and tag_keep_prob < 1:
                tag_mask = tf.nn.dropout(tf.ones(noise_shape), 1 - (tag_keep_prob)) * tag_keep_prob
            else:
                tag_mask = 1
            if rel_inputs is not None and rel_keep_prob < 1:
                rel_mask = tf.nn.dropout(tf.ones(noise_shape), 1 - (rel_keep_prob)) * rel_keep_prob
            else:
                rel_mask = 1
    ...
    def RNN(self, inputs):
        ...
        lengths = tf.reshape(tf.cast(self.sequence_lengths, dtype=tf.int64), [-1])
        ...
        if self.recur_bidir:
            ...
            top_recur.set_shape([tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None),
                                 tf.compat.v1.Dimension(2 * self.recur_size)])
        else:
            top_recur, end_recur = rnn.dynamic_rnn(cell, inputs,
                                                   ...
                                                   dtype=tf.float32)
            top_recur.set_shape(
                [tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(self.recur_size)])
    ...
    def soft_attn(self, top_recur):
        ...
        top_recur.set_shape(
            [tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(4 * self.recur_size)])
        return top_recur
    ...
    def linear(self, inputs, output_size, n_splits=1, add_bias=False):
        ...
        bucket_size = tf.shape(input=inputs)[1]
        input_size = inputs.get_shape().as_list()[-1]
        output_shape = tf.stack([batch_size] + [bucket_size] * (n_dims - 2) + [output_size])
        shape_to_set = [tf.compat.v1.Dimension(None)] * (n_dims - 1) + [tf.compat.v1.Dimension(output_size)]
        ...
        if keep_prob < 1:
            ...
            inputs = tf.nn.dropout(inputs, 1 - (keep_prob), noise_shape=noise_shape)
    ...
    def MLP(self, inputs, output_size, func=None, keep_prob=None, n_splits=1):
        ...
        if func.__name__ in ('gated_tanh', 'gated_identity'):
            linear = [tf.concat([lin1, lin2], n_dims - 1) for lin1, lin2 in
                      zip(linear[:len(linear) // 2], linear[len(linear) // 2:])]
        if n_splits == 1:
            linear = [linear]
    ...
    def double_MLP(self, inputs, n_splits=1):
        ...
        shape_to_set = [tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None),
                        tf.compat.v1.Dimension(output_size)]
        ...
        lin1, lin2 = linalg.linear(inputs,
                                   ...
                                   add_bias=True,
                                   moving_params=self.moving_params)
        lin1 = tf.reshape(tf.transpose(a=lin1, perm=[0, 2, 1]), tf.stack([-1, bucket_size, 1]))
        lin2 = tf.reshape(tf.transpose(a=lin2, perm=[0, 2, 1]), tf.stack([-1, 1, bucket_size]))
        lin = lin1 + lin2
        lin = tf.reshape(lin, tf.stack([batch_size, n_splits * output_size, bucket_size, bucket_size]))
        lin = tf.transpose(a=lin, perm=[0, 2, 3, 1])
    ...
    def linear_classifier(self, inputs, n_classes, add_bias=True, keep_prob=None):
        ...
        output.set_shape([tf.compat.v1.Dimension(None)] * (n_dims - 1) + [tf.compat.v1.Dimension(output_size)])
        return output
    ...
    def bilinear_classifier(self, inputs1, inputs2, add_bias1=True, add_bias2=False, keep_prob=None):
        ...
        input_shape = tf.shape(input=inputs1)
        batch_size = input_shape[0]
        bucket_size = input_shape[1]
        ...
        if isinstance(keep_prob, tf.Tensor) or keep_prob < 1:
            ...
            inputs1 = tf.nn.dropout(inputs1, 1 - (keep_prob), noise_shape=noise_shape)
            inputs2 = tf.nn.dropout(inputs2, 1 - (keep_prob), noise_shape=noise_shape)
    ...
    def diagonal_bilinear_classifier(self, inputs1, inputs2, add_bias1=True, add_bias2=False):
        ...
        if isinstance(keep_prob, tf.Tensor) or keep_prob < 1:
            noise_shape = tf.stack([batch_size, 1, input_size])
            inputs1 = tf.nn.dropout(inputs1, 1 - (tf.sqrt(keep_prob)), noise_shape=noise_shape)
            inputs2 = tf.nn.dropout(inputs2, 1 - (tf.sqrt(keep_prob)), noise_shape=noise_shape)
    ...
    # =============================================================
    def conditional_diagonal_bilinear_classifier(self, inputs1, inputs2, n_classes, probs, add_bias1=True,
                                                 add_bias2=True):
        """"""
        ...
        bucket_size = input_shape[1]
        input_size = inputs1.get_shape().as_list()[-1]
        input_shape_to_set = [tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), input_size + 1]
    ...
    def output(self, logits3D, targets3D):
        ...
        original_shape = tf.shape(input=logits3D)
        ...
        predictions1D = tf.cast(tf.argmax(input=logits2D, axis=1), dtype=tf.int32)
        probabilities2D = tf.nn.softmax(logits2D)
        cross_entropy1D = tf.nn.sparse_softmax_cross_entropy_with_logits(logits2D, targets1D)

        correct1D = tf.cast(tf.equal(predictions1D, targets1D), dtype=tf.float32)
        n_correct = tf.reduce_sum(input_tensor=correct1D * tokens_to_keep1D)
        accuracy = n_correct / self.n_tokens
        loss = tf.reduce_sum(input_tensor=cross_entropy1D * tokens_to_keep1D) / self.n_tokens
    ...
    def conditional_probabilities(self, logits4D, transpose=True):
        ...
        if transpose:
            logits4D = tf.transpose(a=logits4D, perm=[0, 1, 3, 2])
        original_shape = tf.shape(input=logits4D)
