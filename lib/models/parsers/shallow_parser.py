...
class ShallowParser(BaseParser):
  ...
  def __call__(self, dataset, moving_params=None):
    ...
    self.tokens_to_keep3D = tf.expand_dims(tf.cast(tf.greater(inputs[:, :, 0], vocabs[0].ROOT), dtype=tf.float32), 2)
    self.sequence_lengths = tf.reshape(tf.reduce_sum(input_tensor=self.tokens_to_keep3D, axis=[1, 2]), [-1, 1])
    ...
    if self.word_l2_reg > 0:
      unk_mask = tf.expand_dims(tf.cast(tf.greater(inputs[:, :, 1], vocabs[0].UNK), dtype=tf.float32), 2)
    ...
    with tf.compat.v1.variable_scope('Rels', reuse=reuse):
      rel_logits, rel_logits_cond = self.conditional_bilinear_classifier(top_recur, top_recur, len(vocabs[2]),
                                                                         predictions)
      rel_output = self.output(rel_logits, targets[:, :, 2])
      rel_output['probabilities'] = self.conditional_probabilities(rel_logits_cond)

    output = {}
    output['probabilities'] = tf.tuple(tensors=[arc_output['probabilities'],
    ...
    output['n_correct'] = tf.reduce_sum(input_tensor=output['correct'])
