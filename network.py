...
class Network(Configurable):
    ...
    def __init__(self, model, *args, **kwargs):
        ...
        self._save_vars = filter(lambda x: u'Pretrained' not in x.name, tf.compat.v1.all_variables())
        self.history = {
            'train_loss': [],
            'train_accuracy': [],
            'valid_loss': [],
            ...
            while total_train_iters < train_iters:
                for j, (feed_dict, _) in enumerate(self.train_minibatches()):
                    ...
                    if total_train_iters == 1 or total_train_iters % validate_every == 0:
                        ...
                        with open(os.path.join(self.save_dir, 'sanitycheck.txt'), 'w') as f:
                            for k, (feed_dict, _) in enumerate(self.valid_minibatches()):
                                ...
                                start_time = time.time()
                                loss, n_correct, n_tokens, predictions = sess.run(self.ops['valid_op'],
                                                                                  feed_dict=feed_dict)
                    ...
                    if print_every and total_train_iters % print_every == 0:
                        ...
                        print(
                            '%6d) Train loss: %.4f    Train acc: %5.2f%%    Train rate: %6.1f sents/sec\n\tValid loss: %.4f    Valid acc: %5.2f%%    Valid rate: %6.1f sents/sec' % (
                            total_train_iters, train_loss, train_accuracy, train_time, valid_loss, valid_accuracy,
                            valid_time))
                        train_time = 0
                        train_loss = 0
                        n_train_sents = 0
    ...
    with tf.compat.v1.Session(config=config_proto) as sess:
        ...
        if not (args.test or args.matrix):
            if args.load:
                ...
                saver = tf.compat.v1.train.Saver(var_list=network.save_vars)
